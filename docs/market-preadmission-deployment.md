...
新密码为 8–64 位可打印字符，至少包含一个大写字母、一个小写字母、一个数字和一个特殊字符，不得与旧密码相同。修改成功后旧密码立即失效，当前页面保持登录。用户忘记旧密码时由管理员在 CloudBase 身份认证控制台重置，本版本不提供短信或邮箱找回。密码只由 CloudBase Auth 处理，不进入业务云函数、数据库或日志。
...
13. 登录后进入“修改密码”，确认弱密码和两次输入不一致不会调用 CloudBase；使用正确旧密码改为临时强密码，再立即改回原密码，期间不得在命令、截图或日志中显示密码。
