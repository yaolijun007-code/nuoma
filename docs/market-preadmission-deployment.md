...
1. 在 CloudBase 身份认证控制台创建用户并设置用户名和初始密码。
...
管理员账号把 `role` 设为 `admin`。初始密码不得与账号相同，不在群聊中发送密码；首次交付后，由使用人在顶部账号区选择“修改密码”，输入旧密码和两次新密码完成更换。

新密码为 4–64 位可打印字符，可以使用纯数字，不得与旧密码相同。20 个市场账号的初始密码使用各自独立的 6 位数字。修改成功后旧密码立即失效，当前页面保持登录。用户忘记旧密码时由管理员在 CloudBase 身份认证控制台重置，本版本不提供短信或邮箱找回。密码只由 CloudBase Auth 处理，不进入业务云函数、数据库或日志。
...
13. 登录后进入“修改密码”，确认少于 4 位的密码和两次输入不一致不会调用 CloudBase；使用正确旧密码改为临时 4 位数字密码，再立即改回原密码，期间不得在命令、截图或日志中显示密码。
