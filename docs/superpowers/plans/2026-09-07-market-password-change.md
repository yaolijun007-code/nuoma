...
The market build must fail before bundling when either `VITE_CLOUDBASE_ENV_ID` or `VITE_CLOUDBASE_ACCESS_KEY` is missing, so a login-disabled bundle cannot be deployed silently.
