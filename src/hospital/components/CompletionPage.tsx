...
export function CompletionPage({ hasRedFlag, confirmationId }: { hasRedFlag: boolean; confirmationId: string }) {
  const collected = ["基础健康信息", "身体功能状态", "胃肠与生活方式", "男性健康信息", "医学安全信息"];
  return (
    <main className="hospital-completion">
      ...
      <section className="next-flow" id="next-steps" aria-label="后续流程">
        ...
        <div className="flow-step"><span>02</span><strong>微生态检测</strong></div>
        <ArrowDown className="flow-arrow" aria-hidden="true" />
        <div className="flow-step"><span>03</span><strong>综合健康报告</strong></div>
      </section>

      <p className="completion-disclaimer">本次信息将与微生态检测数据及已有健康资料进行综合分析，不依据问卷单独判断疾病或生物年龄。</p>
